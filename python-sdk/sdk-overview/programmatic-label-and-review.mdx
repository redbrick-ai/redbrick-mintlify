...
2. Retrieving the `task_id` from your filename/custom `name` from the Items List using [search\_tasks](exporting-annotations#query-tasks-in-your-project).
3. Retrieving tasks assigned to your API key using `list_tasks`.

#### Programmatically Label Tasks

Add your annotations within the `series` field, along with the `task_id`. Please refer to the reference documentation for the [format of the annotations in Series](../formats/full-format-reference).
...
---
title: Submit tasks with new labels
---
project.labeling.put_tasks("Label", tasks)

---
title: Save tasks as draft with new labels
---
...
```python
---
title: Set review_result to True if you want to accept the tasks
---
project.review.put_tasks("Review_1", [{taskId: "..."}], review_result=True)

---
title: Set review_result to False if you want to reject the tasks
---
project.review.put_tasks("Review_1", [{taskId: "..."}], review_result=False)

---
title: Add labels if you want to accept the tasks with correction
---
...
First, get a list of the `task_id`s you want to send back to Label. You can do this by [exporting only Ground Truth Tasks](exporting-annotations#export-only-ground-truth) and filtering them. Then, use `move_tasks_to_start` to send them back to Label.
